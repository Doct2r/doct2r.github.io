...
## [2026-07-03] 퍼스널 컬러 확정 및 CSS 적용

### 브랜드 컬러 (테마가 바뀌어도 반드시 유지)

| 순서 | 색상 | 헥스 | 용도 |
|------|------|------|------|
| 1번 | 딥 틸 그린 | `#24826E` | 헤더 그라디언트 시작, 링크 호버, 강조 |
| 2번 | 딥 마론 | `#882426` | 헤더 그라디언트 끝, 코드 블록 라인 |

### 적용 규칙
- 헤더 배경: `#24826E → #882426` 135도 그라디언트
- 다크모드: 동일 계열의 어두운 변형값 사용
- **테마를 변경하거나 CSS를 초기화해도 이 두 색을 기준으로 재적용할 것**

### 변경 내용
- `static/css/custom.css`: 기존 잘못된 색(`#4ab89d`, `#308572`)을 퍼스널 컬러로 교체
