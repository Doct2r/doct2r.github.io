...
새 포스트 본문이 완성되고 팩트체크가 끝난 뒤, `POST_INDEX.md`에 넣을 밀집 요약 엔트리(공백 없는 키워드 나열 형식)는 `post-index-haiku` 스킬(`.claude/skills/post-index-haiku/SKILL.md`)로 하이쿠 서브에이전트에 백그라운드로 위임한다.
