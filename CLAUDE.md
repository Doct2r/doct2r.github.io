...
- 족보처럼 세대가 위→아래로 쭉 이어지는 트리형 데이터(`hub/src/pages/graph/abraham.astro`, `hub/src/data/abraham-graph.json` — 데라~야곱의 12아들)는 다른 그래프의 fcose(힘 기반 분산) 대신 Cytoscape 내장 `breadthfirst` 레이아웃(`directed: true`, `roots: [...]`)을 쓴다 — 별도 플러그인 설치 없이 세대 깊이를 그대로 층으로 쌓아준다. 배우자/첩 관계도 방향 있는 엣지로 넣기 때문에 트리 순회에 걸려, 배우자는 대체로 배우자의 자녀와 같은 층 근처에 놓인다(가계도로서 완전히 정확하진 않지만 가벼운 근사로 충분). 새 트리형 관계도를 추가할 때도 이 레이아웃 선택을 기본으로 검토할 것.
